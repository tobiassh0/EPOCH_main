...
def calcNyqists2D(length, nx, duration, nt, angular=True):
	"""
	INPUT
...
#	print("knorm", norm[0])
#	print("wnorm", norm[1])
	deltak, deltaf, knyq, fnyq = calcNyqists2D(length, nx, duration, nt) # calculates nyq values for whole grid, need nyq values for size of FFT 

	deltak = deltak/norm[0]
	knyq = knyq/norm[0]
#	deltaf = deltaf/norm[1]
	deltaf = (2.0*pi/(dt*nfft))/norm[1]
#	fnyq = fnyq/norm[1]
	fnyq = 0.5*deltaf*nfft	
...
def plot_bicoh(bicoh, extent=None, smooth=False, cbar=False, clim=(None, None), cmap ='jet'):
	if (smooth):
		bicoh = gaussian_filter(bicoh, sigma=1) 
	bicoh = np.transpose(np.tril(np.ones(bicoh.shape)))*bicoh
	bicohplot = np.ma.masked_where(bicoh < 0.001, bicoh)
	
	fig,ax = plt.subplots()
	ax.imshow(bicohplot, interpolation='nearest', origin='lower', aspect='auto', extent=extent, cmap=cmap, clim=clim)
